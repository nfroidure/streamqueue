    ...
    describe('for ' + version + ' streams', () => {
      ...
      describe('in binary mode', () => {
        ...
        describe('and with async streams', () => {
          ...
          it('should work with POO API', (done) => {
            ...
            queue.pipe(StreamTest[version].toText((err, text) => {
              if(err) {
                done(err);
                return;
              }
        ...
        describe('and with functions returning streams', () => {
          ...
          if('v2' === version) {
            it('should reemit errors', (done) => {
              ...
              queue.queue(StreamTest[version].fromErroredChunks
                .bind(null, new Error('Aouch!'), []));
              queue.queue(StreamTest[version].fromChunks.bind(null, ['wa', 'dup']));
              queue.queue(StreamTest[version].fromChunks.bind(null, ['pl', 'op']));
              queue.queue(StreamTest[version].fromChunks.bind(null, ['ki', 'koo', 'lol']));
              assert.equal(queue.length, 4);
      ...
      describe('in object mode', () => {
        ...
        it('should work', (done) => {
          ...
          queue.pipe(StreamTest[version].toObjects((err, objs) => {
            if(err) {
              done(err);
              return;
            }
            assert.deepEqual(objs, [
              { s: 'wa' }, { s: 'dup' }, { s: 'pl' }, { s: 'op' },
              { s: 'ki' }, { s: 'koo' }, { s: 'lol' },
            ]);
            done();
          }));
